...
/** Aplica uma ordem manual salva pelo organizador ao ranking geral. */
export function aplicarAjusteLeaderboard(
  ranking: RankingGeral[],
  ajustes: { dupla_id: string; posicao: number }[]
): RankingGeral[] {
  if (!ajustes.length) return ranking;
  const ordem = new Map(ajustes.map((a) => [String(a.dupla_id), a.posicao]));
  return [...ranking].sort((a, b) => {
    const pa = ordem.get(String(a.dupla_id));
    const pb = ordem.get(String(b.dupla_id));
